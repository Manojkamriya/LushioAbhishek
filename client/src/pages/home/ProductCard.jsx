...
function ProductCard(props) {
  ...
  const { addToCart, addToWishlist, removeFromWishlist } = useContext(ShopContext);
    ...
    useEffect(() => {
      if ( props.data && props.data.aboveHeight && props.data.aboveHeight.colorOptions.length > 0) {
    ...
    <div className="card" style={{ aspectRatio: 34 / 61, position: "relative" }}>
     ...
     <div className="size-selector-my" >
        ...
        </button>:<button   className="add-to-cart-button" style={{backgroundColor: "#5f5e5e", cursor: "not-allowed"}}>Select size to add to cart</button>
         }
      ...
      <div className="item-naming">
        ...
        {/* {  isHeightBased  && <p>Height Category: {heightCategory}</p>}
        <p style={{color: selectedColor.hex}}>Selected Color: {selectedColor?.name || 'None'}</p>
